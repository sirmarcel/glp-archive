...
for x64 in [True, False]:
    ...
    initial_atoms = bulk("Ar", cubic=False) * [8, 8, 8]
    ...
    for k in ["energy", "forces", "stress", "heat_flux"]:
        results[f"{k}_{name}"] = []

    for atoms in samples:
        atoms.calc = reference_calculator
        results[f"energy_{name}"].append(atoms.get_potential_energy())
        results[f"forces_{name}"].append(atoms.get_forces())
        results[f"stress_{name}"].append(
            atoms.get_stress(voigt=False) * atoms.get_volume()
        )
        ...
        results[f"heat_flux_{name}"].append(
            np.einsum("iab,ib", stresses, atoms.get_velocities())
        )
